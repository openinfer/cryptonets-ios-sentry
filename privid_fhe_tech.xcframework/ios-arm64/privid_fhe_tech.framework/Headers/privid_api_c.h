...
/**
* Detects and validates face on the input image; if the face is valid, calculate embedding and return it as string.
* The result is returned in the JSON format for future parsing. In case of success, it contains the embedding for future usage.
...
*/
PRIVID_API_ATTRIB int32_t privid_validate_face(
    void* session_ptr, const char* user_config, const int user_config_length,
    const uint8_t* image_bytes, const int image_width, const int image_height,
    char** result_out, int* result_out_length);

/**
* Detects and validates face on the input image; if the face is valid, try to add the user to the cloud database.
...
/**
* Detects and validates face on the input image; if the face is valid, try to find the user in the cloud database.
* The result is returned in the JSON format for future parsing. In case of success, it contains the user UUID for future usage.
* The input image format (RGB/BGR/RGBA) is controlled by the #user_config value or the previous #privid_set_configuration call.
* After usage, the pointer returned in the #resul_out parameter should be deallocated using the #privid_free_char_buffer call.
*
...
/**
* Deletes the user using the #uuid parameter from the cloud database. The result is returned in the JSON format for future parsing.
* After usage, the pointer returned in the #resul_out parameter should be deallocated using the #privid_free_char_buffer call.
*
* \param[in]    session_ptr             The session pointer which obtained from the function #privid_initialized_session call.
* \param[in]    user_config             The UTF-8 encoded JSON string with configuration parameters applied for this operation only.
* \param[in]    user_config_length      The size of the #user_config string in bytes.
* \param[in]    uuid                    The UTF-8 encoded unique user identifier previously obtained from the enroll or predict call.
* \param[in]    uuid_length             The size of the #uuid string in bytes.
* \param[out]   result_out              The output buffer for the UTF-8 encoded JSON string with operation results.
* \param[out]   result_out_length       The size of the #result_out string in bytes.
*
* \return       Operation identifier in case of successful completion or negative value in case of errors.
...
/**
* Compares Base64 encoded embedding with detected face and calculates similarity score based on distance between two embeddings.
* After usage, the pointer returned in the #resul_out parameter should be deallocated using the #privid_free_char_buffer call.
*
* \param[in]    session_ptr             The session pointer which obtained from the function #privid_initialized_session call.
* \param[in]    user_config             The UTF-8 encoded JSON string with configuration parameters applied for this operation only.
* \param[in]    user_config_length      The size of the #user_config string in bytes.
* \param[in]    embedding_one           The Base64 encoded embedding previously obtained from the enroll/predict/validate call.
* \param[in]    embedding_one_length    The size of the #embedding_one string in bytes.
* \param[in]    embedding_two           The Base64 encoded embedding previously obtained from the enroll/predict/validate call.
...
* \param[in]    image_width             The pixel width of the #image_bytes input image.
* \param[in]    image_height            The pixel height of the #image_bytes input image.
* \param[in]    embedding               The Base64 encoded embedding previously obtained from the enroll or predict call.
* \param[in]    embedding_length        The size of the #embedding_one string in bytes.
* \param[out]   result_out              The output buffer for the UTF-8 encoded JSON string with operation results.
* \param[out]   result_out_length       The size of the #result_out string in bytes.
*
* \return       Operation identifier in case of successful completion or negative value in case of errors.
*/
PRIVID_API_ATTRIB int32_t privid_compare_face_and_embedding(
    void* session_ptr, const char* user_config, const int user_config_length,
    const uint8_t* image_bytes, const int image_width, const int image_height,
    const char *embedding, const int embedding_length,
    char** result_out, int* result_out_length);

/**
* Compares mughost from document with detected face and calculates similarity score based on distance between two embeddings.
* After usage, the pointer returned in the #resul_out parameter should be deallocated using the #privid_free_char_buffer call.
*
* \param[in]    session_ptr             The session pointer which obtained from the function #privid_initialized_session call.
* \param[in]    user_config             The UTF-8 encoded JSON string with configuration parameters applied for this operation only.
* \param[in]    user_config_length      The size of the #user_config string in bytes.
* \param[in]    doc_image_bytes         The input document image is a raw bytes pixel buffer (bitmap).
* \param[in]    doc_image_width         The pixel width of the #doc_image_bytes input image.
* \param[in]    doc_image_height        The pixel height of the #doc_image_bytes input image.
* \param[in]    face_image_bytes        The input face image is a raw bytes pixel buffer (bitmap).
* \param[in]    face_image_width        The pixel width of the #face_image_bytes input image.
* \param[in]    face_image_height       The pixel height of the #face_image_bytes input image.
* \param[out]   result_out              The output buffer for the UTF-8 encoded JSON string with operation results.
* \param[out]   result_out_length       The size of the #result_out string in bytes.
*
* \return       Operation identifier in case of successful completion or negative value in case of errors.
*/
PRIVID_API_ATTRIB int32_t privid_compare_mugshot_and_face(
    void* session_ptr, const char* user_config, const int user_config_length,
    const uint8_t* doc_image_bytes, const int doc_image_width, const int doc_image_height,
    const uint8_t* face_image_bytes, const int face_image_width, const int face_image_height,
    char** result_out, int* result_out_length);
